...
const MarkdownContent = memo(function MarkdownContent({ 
  ...
  const processedContent = useMemo(() => {
    ...
    for (const highlight of highlightsWithPositions) {
      ...
      const colorAttribute = highlight.color ? ` data-color="${highlight.color}"` : '';
      result = `${before}<mark class="explored-highlight" data-id="${highlight.id}"${colorAttribute}>${matchedText}</mark>${after}`;
    }
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          mark: ({ node, children, ...props }: any) => {
            ...
            const color = typeof props['data-color'] === 'string' ? props['data-color'] : undefined;
            ...
            if (isExploredHighlight && id) {
              return (
                <mark
                  ...
                  style={{ backgroundColor: color || 'rgba(168, 85, 247, 0.5)', color: 'white' }}
                >
                  {children}
                </mark>
              );
